...
ALL slug params are lowercase-hyphenated — NEVER guess, always call ws_list_* first.

## Flow 1: Make → Model → Year (default — user names a vehicle)
1. ws_list_makes() → find make slug
2. ws_list_models(make) → find model slug
3. ws_list_years(make, model) → find year
4. ws_list_modifications(make, model, year) → find modification slug
5. ws_search_by_vehicle(make, model, year, modification=<slug>)

## Flow 2: Make → Year → Model (user knows brand + year, not model)
1. ws_list_makes() → find make slug
2. ws_list_years(make) → find year
3. ws_list_models(make, year) → find model slug
4. ws_list_modifications(make, model, year) → find modification slug
5. ws_search_by_vehicle(make, model, year, modification=<slug>)

## Flow 3: Year → Make → Model (year-first — common for tire shops)
1. ws_list_years() → confirm year exists
2. ws_list_makes(year) → find make slug
3. ws_list_models(make, year) → find model slug
4. ws_list_modifications(make, model, year) → find modification slug
5. ws_search_by_vehicle(make, model, year, modification=<slug>)
...
1. ws_list_makes() → find make slug
2. ws_list_models(make) → find model slug
3. ws_list_generations(make, model) → find generation slug
4. ws_list_modifications(make, model, generation=<slug>) → find modification
5. ws_search_by_vehicle(make, model, generation=<slug>, modification=<slug>)
...
- ws_search_by_vehicle REQUIRES TWO conditions:
...
3. Call ws_find_vehicles_for_rim(
     bolt_pattern='{bolt_pattern}', rim_diameter={rim_diameter},
     rim_width={rim_width}, rim_offset={rim_offset}
...
4. For any interesting generation, drill down with
   ws_find_vehicle_modifications_for_rim to see per-trim fitment deltas.
...
1. Call ws_get_spec_metadata with all available params to get geometry,
   population stats, and hints.
"""
        if section_width and aspect_ratio:
            steps += (
                f"2. Call ws_find_vehicles_for_package(\n"
            ...
            steps += (
                f"2. Call ws_find_vehicles_for_rim(\n"
                f"     bolt_pattern='{bolt_pattern}',\n"
                f"     rim_diameter={rim_diameter},\n"
                f"     rim_width={rim_width},\n"
                f"     rim_offset={rim_offset}\n"
                f"   ) for compatible vehicles.\n"
                f"3. Call ws_find_tires_for_rim with the same rim params\n"
