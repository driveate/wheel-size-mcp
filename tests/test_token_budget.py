...
async def test_list_makes_all(call_tool):
    """ws_list_makes returns all 246+ makes — must stay under budget."""
    from ws_mcp.server import mcp

    result = await mcp.call_tool("ws_list_makes", {})
    text = result.content[0].text
    data = json.loads(text)
    assert data["total"] > 200
    assert_under_token_cap(text, "ws_list_makes (all)")
...
async def test_list_models_largest_make(call_tool):
    ...
    result = await mcp.call_tool("ws_list_models", {"make": "toyota"})
    text = result.content[0].text
    data = json.loads(text)
    assert data["total"] > 200
    assert_under_token_cap(text, "ws_list_models (toyota)")
...
async def test_list_modifications_many_trims(call_tool):
    ...
    result = await mcp.call_tool("ws_list_modifications", {
        "make": "bmw", "model": "3-series", "year": 2024,
    })
    text = result.content[0].text
    data = json.loads(text)
    assert data["total"] > 30
    assert_under_token_cap(text, "ws_list_modifications (BMW 3-series)")
...
async def test_search_by_vehicle_full_detail_heavy(call_tool):
    ...
    assert_under_token_cap(text, "ws_search_by_vehicle (BMW 3-series full)")


async def test_search_by_rim_max_limit(call_tool):
    """ws_search_by_rim with limit=50 — max page size."""
    from ws_mcp.server import mcp

    result = await mcp.call_tool("ws_search_by_rim", {
        "bolt_pattern": "5x114.3", "rim_diameter": 18, "rim_width": 8, "limit": 50,
    })
    text = result.content[0].text
    assert_under_token_cap(text, "ws_search_by_rim (limit=50)")
...
async def test_find_vehicles_for_rim_max_limit(call_tool):
    """ws_find_vehicles_for_rim with limit=50 — enriched response with deltas."""
    from ws_mcp.server import mcp

    result = await mcp.call_tool("ws_find_vehicles_for_rim", {
    ...
    assert_under_token_cap(text, "ws_find_vehicles_for_rim (limit=50)")
